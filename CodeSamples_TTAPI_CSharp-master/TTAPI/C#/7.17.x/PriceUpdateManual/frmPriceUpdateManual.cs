...
namespace TTAPI_Samples
{
    ...
    public partial class frmPriceUpdateManual : Form
    {
        ...
        private PriceSubscription m_priceSubscription2 = null;
        ...
        public void shutdownTTAPI()
        {
            if (!m_shutdownInProcess)
            {
                ...
                {
                    m_priceSubscription1.FieldsUpdated -= priceSubscription_FieldsUpdated;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.cboProductType2.DataSource = ProductType.AllAvailableValues.ToList();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Click event which will initiate the price subscription1
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn1Connect_Click(object sender, EventArgs e)
        {
            ...
            instrRequest.Update += instr1Request_Completed;
            instrRequest.Start();
        }

        /// <summary>
        /// Click event which will initiate the price subscription2
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn2Connect_Click(object sender, EventArgs e)
        {
            ...
            // Create a product key from the given values
            ProductKey key = new ProductKey(this.txtExchange2.Text,
                                            (ProductType)this.cboProductType2.SelectedItem,
                                            this.txtProduct2.Text);

            // Find out instrument based on the previously created key and the contract name
            InstrumentLookupSubscription instrRequest = new InstrumentLookupSubscription(this.m_TTAPI.Session, Dispatcher.Current, key, this.txtContract2.Text);
            instrRequest.Update += instr2Request_Completed;
            instrRequest.Start();
        }

        /// <summary>
        /// Instrument1 request completed event.
        /// </summary>
        void instr1Request_Completed(object sender, InstrumentLookupSubscriptionEventArgs e)
        {
            instrRequest_Completed(sender, e, 1);
        }

        /// <summary>
        /// Instrument2 request completed event.
        /// </summary>
        void instr2Request_Completed(object sender, InstrumentLookupSubscriptionEventArgs e)
        {
            instrRequest_Completed(sender, e, 2);
        }

        /// <summary>
        /// Generic Instrument request completed event.
        /// </summary>
        void instrRequest_Completed(object sender, InstrumentLookupSubscriptionEventArgs e, int psNum)
        {
            if (e.IsFinal && e.Instrument != null)
            {
                try
                {
                    ...
                    if(psNum == 1)
                    {
                        ...
                        m_priceSubscription1 = new PriceSubscription(e.Instrument, Dispatcher.Current);
                        m_priceSubscription1.Settings = new PriceSubscriptionSettings(PriceSubscriptionType.InsideMarket);
                        m_priceSubscription1.FieldsUpdated += new FieldsUpdatedEventHandler(priceSubscription_FieldsUpdated);
                        m_priceSubscription1.Start();
                    }
                    else
                    {
                        if (m_priceSubscription2 != null)
                        {
                            m_priceSubscription2.Dispose();
                            m_priceSubscription2 = null;
                        }

                        // subscribe for price updates
                        m_priceSubscription2 = new PriceSubscription(e.Instrument, Dispatcher.Current);
                        m_priceSubscription2.Settings = new PriceSubscriptionSettings(PriceSubscriptionType.InsideMarket);
                        m_priceSubscription2.FieldsUpdated += new FieldsUpdatedEventHandler(priceSubscription_FieldsUpdated);
                        m_priceSubscription2.Start();
                    }
                }
                catch (Exception err)
                {
        ...
        void priceSubscription_FieldsUpdated(object sender, FieldsUpdatedEventArgs e)
        {
            ...
            if (this.m_priceSubscription1 != null && String.Equals(ps.Instrument.Name, this.m_priceSubscription1.Instrument.Name))
            {
            ...
            {
                // Extract the values we want as Typed fields
                this.txtBidPrice2.Text = e.Fields.GetDirectBidPriceField().FormattedValue;
                this.txtBidQty2.Text = e.Fields.GetDirectBidQuantityField().FormattedValue;
                this.txtAskPrice2.Text = e.Fields.GetDirectAskPriceField().FormattedValue;
                this.txtAskQty2.Text = e.Fields.GetDirectAskQuantityField().FormattedValue;
                this.txtLastPrice2.Text = e.Fields.GetLastTradedPriceField().FormattedValue;
                this.txtLastQty2.Text = e.Fields.GetLastTradedQuantityField().FormattedValue;  
            }
        }
