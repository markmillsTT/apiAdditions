...
namespace TTAPI_Samples
{
    public partial class frmSOD_ManualFill : Form
    {
        ...
        private System.Collections.ObjectModel.ReadOnlyCollection<OrderFeed> OrderFeeds;
        ...
        /*
        private void ProcessAndPublishSODRecord(InstrumentCatalogSubscription catalog)
        {
            String _updateInfo;
            StartOfDayRequiredFields requiredFields = new StartOfDayRequiredFields();
            try
            {
                ...
                //if (catalog.Product.Type == ProductType.Option)
                //{

                //    _instrument = catalog.Instruments.Values.First(
                //    i => (i.InstrumentDetails.StrikePriceDecimals > 0) ?
                //    i.GetFormattedName(InstrumentNameFormat.Expiry) ==
                //    m_pic.instrumentName.Substring(0, (m_pic.instrumentName.IndexOf('.') + i.InstrumentDetails.StrikePriceDecimals + 1)) :
                //    i.GetFormattedName(InstrumentNameFormat.Expiry) ==
                //    m_pic.instrumentName.Substring(0, (m_pic.instrumentName.IndexOf('.')))
                //    );
                //}
                //else
                    _instrument = catalog.Instruments.Values.First(i => i.GetFormattedName(InstrumentNameFormat.Expiry) == m_pic.instrumentName);

                String _errorMessage;

                OrderFeeds = _instrument.GetValidOrderFeeds();

                bool _gatewayMatched = false;


                foreach (OrderFeed orderFeed in OrderFeeds)
                {
                    //if (orderFeed.Name == m_pic.productKey.MarketKey.Name
                    if (!(orderFeed.Name.Contains("CME") || orderFeed.Name.Contains("TOCOM") || orderFeed.Name.Contains("ICE_IPE")))
                    {
                        m_pic.riskAccount = orderFeed.ConnectionKey.MemberId;
                    }

                    //_updateInfo = "OF: Gateway Key: " + orderFeed.ConnectionKey.GatewayKey + " / Member ID: " + orderFeed.ConnectionKey.MemberId;
                    //Log4NetWrapper.LogInfo(_updateInfo);
                    //if (TTAPIEventWrapper != null)
                    //    TTAPIEventWrapper(this,
                    //    new ConnectorStatusEventArgs(
                    //    (int)TTAPIEventWrapperEnum.TTApiConnector_PublishSODRecord,
                    //    0,
                    //    _updateInfo));

                    //_updateInfo = "DB: Gateway Key: " + m_pic.gatewayKey + " / Member ID: " + m_pic.riskAccount;
                    //Log4NetWrapper.LogInfo(_updateInfo);
                    //if (TTAPIEventWrapper != null)
                    //    TTAPIEventWrapper(this,
                    //    new ConnectorStatusEventArgs(
                    ...
                    //if (orderFeed.ConnectionKey.GatewayKey == m_pic.gatewayKey && orderFeed.ConnectionKey.MemberId == listMemberID[intCtr])
                    if (orderFeed.ConnectionKey.GatewayKey == m_pic.gatewayKey && orderFeed.ConnectionKey.MemberId == m_pic.riskAccount)
                    {
                        requiredFields.MemberId = m_settings.Member;
                        requiredFields.GroupId = m_settings.Group;
                        requiredFields.TraderId = m_settings.Trader;
                        ...
                        m_pic.requiredFields.InstrumentDetails = _instrument.InstrumentDetails;
                        m_pic.requiredFields.OrderFeed = orderFeed;

                        m_pic.requiredFields.Quantity = Quantity.FromString(_instrument, m_pic.quantity);
                        StartOfDayRecord sodRecord = new StartOfDayRecord(m_pic.requiredFields);

                        //sodRecord.Price = Price.FromString(_instrument, m_pic.price);
                        //sodRecord.RiskAccount = //m_pic.riskAccount;
                        sodRecord.RiskAccount = m_pic.requiredFields.Username;


                        if (!m_apiInstance.Session.RiskManager.Publish(sodRecord, out _errorMessage))
                        {
                            //_updateInfo = "Publishing " + _instrument.Name + " Failed. Account: " + m_pic.riskAccount
                            // + " Gateway: " + m_pic.gatewayKey.Name + ". " + _errorMessage;
                            _updateInfo = "Publishing Failed: " + m_pic.GetInfo() + ". " + _errorMessage;
                            emailParser.ComposeEmailContent(_updateInfo);
                            Log4NetWrapper.LogError("TTAPI-ERROR: " + _updateInfo);
                        ...
                        {
                            totalPositionsPublished++;
                            _updateInfo = "Successfully Published: " + m_pic.GetInfo();

                            Log4NetWrapper.LogInfo("TTAPI-INFO: " + _updateInfo);
                ...
                //if (!_gatewayMatched)
                //{
                //    _updateInfo = "Publishing Failed: " + m_pic.GetInfo()
                //    + ". No order feeds found for the gateway: " + m_pic.gatewayKey.Name;
                ...
                //flagUploadInProgress = false;
                //ProcessNextSODRecord();
            }
            catch (Exception ex)
            {
                //_updateInfo = "Failed to publish SOD Record." + m_pic.GetInfo()
                //+ ". No Matching Instrument Found.";

                //emailParser.ComposeEmailContent(_updateInfo);

                //Log4NetWrapper.LogError("TTAPI-ERROR: " + _updateInfo + " " + ex.Message);
