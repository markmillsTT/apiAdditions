...
namespace TTAPI_Samples
{
    ...
    public partial class frmPriceUpdateDepth : Form
    {
        ...
        void priceSubscription_FieldsUpdated(object sender, FieldsUpdatedEventArgs e)
        {
            ...
            int length = e.Fields.GetChangedFieldIds().Length;
            FieldId[] fieldIds = e.Fields.GetChangedFieldIds();

            if(length == 0)
            {
                Console.WriteLine("Length of 0 found");
                for (int i = 0; i < e.Fields.GetLargestCurrentDepthLevel(FieldId.BestAskPrice); i++)
                {
                    length = e.Fields.GetChangedFieldIds(i).Length;
                    if (length != 0)
                    {
                        fieldIds = e.Fields.GetChangedFieldIds(i);
                        if(fieldIds != null)
                        {
                            Console.Write("");
                        }
            ...
            {
                TickPrice tickPrice = m_priceSubscription.Instrument.InstrumentDetails.TickSize;
                double tickValue = m_priceSubscription.Instrument.InstrumentDetails.TickValue;
                Price fromDouble = Price.FromDouble(m_priceSubscription.Instrument, 3904.0);
                Price fromString = Price.FromString(m_priceSubscription.Instrument, "3904.0");


                Console.WriteLine(fromDouble + " " + fromString);
