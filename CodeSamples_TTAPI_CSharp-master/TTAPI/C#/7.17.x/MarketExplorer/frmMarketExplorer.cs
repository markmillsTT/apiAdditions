...
namespace TTAPI_Samples
{
    ...
    public partial class frmMarketExplorer : Form
    {
        ...
        // [Import from some properties file or user input]
        // ...
        // CME Calendar Info (cme_months[1] == [Jan code], cme_months[2] == [Feb code], etc ...)
        static string[] cme_months = { "", "F", "G", "H", "J", "K", "M", "N", "Q", "U", "V", "X", "Z" };
        // Made this as "input format" b/c this is what I imagine the input would be fed as
        // Interested Contracts ([Market]-[Product]-[First Month]-[Second Month]-[Last Digit Of First Year]-[Last Digit Of Second Year])
        string[] spreads = {"CME-ES-3-6-6-6", "CME-ESI-5-3-6-6"};
        // ...
        // [Import from some properties file or user input]
        // ...
        List<Instrument> m_instruments = new List<Instrument>();
        ...
        void ttapiInstance_ConnectionStatusUpdate(object sender, ConnectionStatusUpdateEventArgs e)
        {
            if (e.Status.IsSuccess)
            {
                ...
                {
                    string[] spreadDetails = spread.Split(new char[] { '-' });
                    ProductLookupSubscription m_pls = new ProductLookupSubscription(m_TTAPI.Session, Dispatcher.Current,
                        new ProductKey(spreadDetails[0], ProductType.Spread, spreadDetails[1]));
                    m_pls.Update += new EventHandler<ProductLookupSubscriptionEventArgs>(m_pls_updated);
                    m_pls.Start();
                }
                //..
        ...
        #region Spread Subsctiptions
        /// <summary>
        /// Called when a ProductLookupSubscription updates
        /// </summary>
        private void m_pls_updated(object sender, ProductLookupSubscriptionEventArgs e)
        {
            // Only check for when a Product is found
            if(e != null && e.Product != null)
            {
                InstrumentCatalogSubscription m_ics = new InstrumentCatalogSubscription(e.Product, Dispatcher.Current);
                m_ics.InstrumentsUpdated += new EventHandler<InstrumentCatalogUpdatedEventArgs>(m_ics_updated);
                m_ics.Start();
            }
        }

        /// <summary>
        /// Called when Instrument Catalog is found for a particular product
        /// </summary>
        private void m_ics_updated(object sender, InstrumentCatalogUpdatedEventArgs e)
        {
            if (e != null)
            {
                InstrumentCatalogSubscription m_ics = (InstrumentCatalogSubscription)sender;
                string productName = m_ics.Product.Name;
                List<string[]> all_spread_details = new List<string[]>();

                // Find which spreads matches this product
                foreach ( string spread in spreads)
                {
                    string [] spread_details_parse = spread.Split(new char[]{'-'});
                    if(String.Equals(spread_details_parse[1], productName))
                    {
                        all_spread_details.Add(spread_details_parse);
                    }
                }

                // should not happen - no spreads match the product name
                if (all_spread_details.ToArray().Length == 0)
                {
                    return;
                }

                foreach (string[] spread_details in all_spread_details)
                {
                    // Find which instrument in the product matches the spread
                    foreach (Instrument inst in m_ics.Instruments.Values)
                    {
                        string instName = inst.GetFormattedName(InstrumentNameFormat.Short);
                        int firstMonth;
                        int secMonth;
                        int firstYear;
                        int secYear;

                        int.TryParse(spread_details[2], out firstMonth);
                        int.TryParse(spread_details[3], out secMonth);
                        int.TryParse(spread_details[4], out firstYear);
                        int.TryParse(spread_details[5], out secYear);

                        // Remove delimiters that commonly appear in spread instrument names
                        ...
                        // Format your desired spread name to instrument
                        string spreadName = spread_details[1] + cme_months[firstMonth] + firstYear +
                            cme_months[secMonth] + secYear;

                        if (String.Equals(spreadName, instName))
                        {
        ...
        private void instrumentsUpdated(object sender, InstrumentCatalogUpdatedEventArgs e)
        {
            ...
            Boolean check = e.Added[0].GetValidOrderFeeds()[0].IsTradingEnabled;
            if(check)
            {
        ...
        private void updateInstrumentTreeViewNodes(InstrumentCatalogSubscription instrumentCatalogSubscription)
        {
            ...
            foreach (Instrument instr in instrumentCatalogSubscription.Instruments.Values)
            {
                ...
                if (instr.InstrumentDetails.TickSize.Numerator == 0 || instr.InstrumentDetails.TickSize.Denominator == 0)
                {
                    MessageBox.Show("Found one: " + instr.GetFormattedName(InstrumentNameFormat.Normal));
                }
                ...
                node.ToolTipText = instr.GetFormattedName(InstrumentNameFormat.Short) + "\r\n"
                    ...
                    + " / " + instr.InstrumentDetails.TickSize.Denominator.ToString();
