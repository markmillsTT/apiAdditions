...
namespace XTAPI_Samples
{
	...
	public class frmOrderSelector : Form
    {
        ...
        private Dictionary<String, XTAPI.TTOrderSetClass> instrName_orderSet = 
            new Dictionary<String, XTAPI.TTOrderSetClass>();
		...
		{         		
			...
			while (tempOrderTrackerObj != null)
			{
				...
				if (tempOrderTrackerObj.HasOldOrder != 0)
				{
                    ...
                    Array tempOrderData = (Array)tempOldOrder.get_Get("Instr,Contract$,BuySell,OrderSource,OrderQty");
			
					//get display data
                    txtOutput.Text = getConsoleOutputFromOrder(tempOrderData);
				...
				if (tempOrderTrackerObj.HasNewOrder != 0)
				{
					...
					Array tempOrderData = (Array) tempNewOrder.get_Get("Instr,Contract$,BuySell,OrderSource,OrderQty");
        ...
        /// 
        /// </summary>
        /// <param name="tempOrderData"></param>
        /// <returns></returns>
        private string getConsoleOutputFromOrder(Array tempOrderData)
        {
            string retVal = "";
            ...
            {
                retVal += tempOrderData.GetValue(i) + ",  ";
                // item 5 or index 4 is OrderQty
                if(i == 4)
                {
